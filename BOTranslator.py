...
@client.command()
async def info(ctx):
    embed = discord.Embed(title="Info", colour=0x008cff)
    embed.add_field(
        name = "Translate",
        value = "Use '--translate (lang_id) (the sentences)'\nExample '--translate ja i love you' -> translate from detection language to Japanese",
        inline= False,
    )
    embed.add_field(
        name = "Language ID",
